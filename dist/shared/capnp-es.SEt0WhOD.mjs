...
import { relative, dirname, normalize, join } from 'node:path/posix';
import { Type, ElementSize, Field, Value, Type_Which, Field_Ordinal, Node, CodeGeneratorRequest } from '../capnp/schema.mjs';
import { M as Message } from './capnp-es.BbvJItGh.mjs';
import { f as format, am as pad } from './capnp-es.DQO_cvul.mjs';
import { t as testWhich, a as getUint16, g as getPointer } from './capnp-es.d92Owwob.mjs';
...
function isLocalNode(ctx, lookup) {
  const id = typeof lookup === "bigint" ? lookup : lookup.id;
  const node = ctx.nodeById.get(id);
  return ctx.localNodeIds.has(id) || node?.displayName.startsWith(`${ctx.schemaDisplayName}:`) === true;
}
function loadRequestedFile(genCtx, file) {
  const ctx = new CodeGeneratorFileContext(genCtx, file);
  const schema2 = lookupNode(ctx, file.id);
  ctx.tsPath = schemaDisplayNameToTsPath(schema2.displayName);
  return ctx;
}
function schemaDisplayNameToTsPath(displayName) {
...
function generateServer(ctx, node) {
  ...
  const extendsClause = superclasses.length === 0 ? "" : ` extends ${superclasses.map((node2) => `${getFullClassName(node2)}$Server$Target`).join(", ")}`;
  ctx.codeParts.push(`
  ...
  const ownMethodsTable = superclasses.length === 0 ? `${clientName}.methods` : `${clientName}.ownMethods`;
  let index = 0;
  for (const method of node.interface.methods) {
    codeServerMethods.push(`{
...
function generateClient(ctx, node) {
  ...
  const inheritedMethodNames = /* @__PURE__ */ new Set();
  for (const superclass of superclasses) {
  ...
  members.push(`
    ...
    ${superclasses.length === 0 ? "" : `static readonly methods: $.Method<any, any>[] = [
      ${superclasses.map((node2) => `...${getFullClassName(node2)}$Client.methods`).join(",\n")},
...
function getInterfaceSuperclasses(ctx, node) {
  return node.interface.superclasses.toArray().map((superclass) => lookupNode(ctx, superclass.id));
}
function generateInheritedClientMethods(ctx, node, methodsCode, seenNames = /* @__PURE__ */ new Set()) {
...
function collectInterfaceMethods(ctx, node, methods = [], seenNodes = /* @__PURE__ */ new Set()) {
...
function generateResultPromise(ctx, node, options = {}) {
  ...
  `);
  const { struct } = node;
  const fields = options.pipelineFields === false ? [] : struct.fields.toArray().sort(compareCodeOrder);
...
function generateClientMethod(ctx, node, clientName, methodsCode, methodDefs, methodDefTypes, index, methodArrayName = "methods") {
...
function createFieldMetadata(ctx, node, field, fieldIndex) {
  const discriminant = field.discriminantValue === Field.NO_DISCRIMINANT ? "" : `, discriminantValue: ${field.discriminantValue}`;
...
function getFieldOrdinal(field, fallback) {
  return field.ordinal.which() === Field_Ordinal.EXPLICIT ? field.ordinal.explicit : fallback;
}
function createTypeMetadata(ctx, type) {
  switch (type.which()) {
    case Type.ANY_POINTER: {
    ...
    default: {
      throw new Error(format(GEN_UNKNOWN_TYPE, type.which()));
    }
  }
}
function createNodeTypeMetadata(kind, node) {
...
class CodeGeneratorContext {
  constructor(req) {
    ...
    this.nodesByScopeId = /* @__PURE__ */ new Map();
...
class CodeGeneratorFileContext {
  constructor(context, file) {
    ...
    this.schemaDisplayName = context.nodeById.get(file.id)?.displayName ?? file.filename;
...
function collectLocalNodeIds(nodesByScopeId, fileId) {
  const localNodeIds = /* @__PURE__ */ new Set();
  const visit = (id) => {
  ...
  for (const imp of ctx.imports) {
    ...
    const imports = getImportNodes(ctx, importNode).flatMap((node) => {
      ...
      if (node._isInterface) {
        const imports2 = [fullClassName, `${fullClassName}$Client`];
        if (importedSuperclassIds.has(node.id)) {
          imports2.push(`type ${fullClassName}$Server$Target`);
        }
        return imports2;
      }
...
function collectImportedSuperclassIds(ctx) {
...
function generatedImportPath(fromTsPath, toSchemaDisplayName) {
...
function tsCompile(files, dts, js, tsconfig) {
  ...
  const resolveInMemoryFile = (filename) => {
